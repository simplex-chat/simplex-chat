...
directoryServiceTests = do
  ...
  describe "member admission" $ do
    ...
    it "should hold captcha-passing member for admin review when admission=all" testCaptchaWithReview
...
-- Regression: group has member admission review = all (admins must review joiners),
-- AND the directory bot screens joiners with a captcha. After a member passes the captcha
-- the bot calls APIAcceptMember; the core must still hold them as pending review
-- (the bot's captcha approval must not bypass admin review).
testCaptchaWithReview :: HasCallStack => TestParams -> IO ()
testCaptchaWithReview ps =
  withDirectoryService ps $ \superUser dsLink ->
    withNewTestChat ps "bob" bobProfile $ \bob ->
      withNewTestChat ps "cath" cathProfile $ \cath -> do
        bob `connectVia` dsLink
        -- create group and enable admin review of all joiners BEFORE the bot joins,
        -- so the bot never processes a profile change (avoids re-approval noise)
        bob ##> "/g privacy Privacy"
        bob <## "group #privacy (Privacy) is created"
        bob <## "to add members use /a privacy <name> or /create link #privacy"
        bob ##> "/set admission review #privacy all"
        bob <## "changed member admission rules"
        bob ##> "/a privacy 'SimpleX Directory' admin"
        bob <## "invitation to join the group #privacy sent to 'SimpleX Directory'"
        welcomeWithLink <- groupAccepted bob "privacy" 1
        completeRegistration superUser bob "privacy" "Privacy" welcomeWithLink 1
        -- get group link
        bob #> "@'SimpleX Directory' /role 1"
        ...
        bob <# "'SimpleX Directory'> > /filter 1 captcha"
        -- captcha is enabled by default for new registrations, so this is a no-op confirm
        bob <## "      Spam filter settings for group privacy:"
        bob <## "- reject long/inappropriate names: disabled"
        ...
        cath <# ("#privacy (support) 'SimpleX Directory'!> > cath " <> captcha)
        cath <## "      Correct, you joined the group privacy"
        -- correct behavior: member is held for admin review, NOT fully admitted
        cath <## "#privacy: 'SimpleX Directory' accepted you to the group, pending review"
        cath <## "#privacy: member bob (Bob) is connected" -- connects to moderator (owner) for review
        -- owner (bob) is prompted to review the member, confirming admin review applies
        bob <## "#privacy: 'SimpleX Directory' added cath (Catherine) to the group (connecting and pending review...), use /_accept member #1 3 <role> to accept member"
        bob <## "#privacy: new member cath is connected and pending review, use /_accept member #1 3 <role> to accept member"
