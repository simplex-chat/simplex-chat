...
simplexLink :: SimplexUriType -> Text -> NonEmpty Text -> Text -> Markdown
simplexLink mode simplexUri smpHosts = Markdown $ Just SimplexUri {mode, simplexUri, smpHosts}
...
textWithUri = describe "text with Uri" do
  ...
  it "SimpleX links" do
    ...
    parseMarkdown ("https://simplex.chat" <> inv) `shouldBe` simplexLink XUriInvitation ("simplex:" <> inv) ["smp.simplex.im"] ("https://simplex.chat" <> inv)
    ...
    parseMarkdown ("https://simplex.chat" <> ct) `shouldBe` simplexLink XUriContact ("simplex:" <> ct) ["smp.simplex.im"] ("https://simplex.chat" <> ct)
    ...
    parseMarkdown ("https://simplex.chat" <> gr) `shouldBe` simplexLink XUriGroup ("simplex:" <> gr) ["smp4.simplex.im", "o5vmywmrnaxalvz6wi3zicyftgio6psuvyniis6gco6bp6ekl4cqj4id.onion"] ("https://simplex.chat" <> gr)
