        ...
        forM_ responses $ \name ->
          case (find ((name ==) . consName') chatResponsesTypeInfo, find ((name ==) . consName') chatResponsesDocs) of
            (Just RecordTypeInfo {fieldInfos}, Just CRDoc {responseDescr}) -> do
              ...
              printFields h "./TYPES.md" fieldInfos
...
printFields :: Handle -> Text -> [FieldInfo] -> IO ()
printFields h docPath =
  ...
  where
    fieldType = \case
      ...
      TIArray {elemType, nonEmpty} -> "[" <> fieldType elemType <> "]" -- <> (if nonEmpty then " // (non-empty)" else "") -- removed because it has to "pop" out of maybe
      TIMap {keyType, valueType} -> "{" <> simpleType keyType <> " : " <> fieldType valueType <> "}"
    simpleType (STI t _ps)
      | t `elem` primitiveTypes = t'
      | otherwise = "[" <> t' <> "](" <> docPath <> "#" <> headerAnchor t' <> ")"
      where
        t' = T.pack t
