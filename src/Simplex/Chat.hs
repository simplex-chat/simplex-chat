...
processChatCommand = \case
  ...
  APISendMessage cType chatId file_ mc -> withUser $ \user@User {userId} -> withChatLock $ case cType of
  ...
  APISendMessageQuote cType chatId quotedItemId file_ mc -> withUser $ \user@User {userId} -> withChatLock $ case cType of
    -- TODO send message with file attachment; initiate file transfer
    CTDirect -> do
...
chatCommandP =
  ("/user " <|> "/u ") *> (CreateActiveUser <$> userProfile)
    ...
    <|> "/_send " *> (APISendMessage <$> chatTypeP <*> A.decimal <*> optFilePath <* A.space <*> msgContentP)
    <|> "/_send_quote " *> (APISendMessageQuote <$> chatTypeP <*> A.decimal <* A.space <*> A.decimal <*> optFilePath <* A.space <*> msgContentP)
  ...
  where
    ...
    optFilePath = do
      f <- (" file" *> A.takeByteString) <|> pure ""
      pure $ if B.null f then Nothing else Just ((T.unpack . safeDecodeUtf8) f)
    memberRole =
      (" owner" $> GROwner)
        <|> (" admin" $> GRAdmin)
        <|> (" member" $> GRMember)
