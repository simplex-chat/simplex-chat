...
newChatController
  ...
  backgroundMode = do
    ...
    logDebug $ "newChatController newTBQueue size: " <> tshow tbqSize
...
sendGroupMessage' user GroupInfo {groupId} members chatMsgEvent = do
  ...
  logDebug $ "sendMessagesB toSend: " <> tshow (length toSend) <> " " <> tshow (length $ nub $ map (groupMemberId' . fst) toSend)
