...
processChatCommand' vr = \case
    ...
    CTDirect -> do
      ...
      if isVoice mc && not (featureAllowed SCFVoice forUser ct)
        then pure $ chatCmdError (Just user) ("feature not allowed " <> T.unpack (chatFeatureNameText CFVoice))
          ...
          (msg, _, _) <- sendDirectContactMessage user ct (XMsgNew msgContainer)
          ...
          case (ciContent, itemSharedMsgId, editable) of
            (CISndMsgContent oldMC, Just itemSharedMId, True) -> do
              ...
              if changed || fromMaybe False itemLive
                then do
                  (SndMessage {msgId}, _, _) <- sendDirectContactMessage user ct (XMsgUpdate itemSharedMId mc (ttl' <$> itemTimed) (justTrue . (live &&) =<< itemLive))
                  ci' <- withStore' $ \db -> do
    ...
    CTDirect -> do
      ...
      case (mode, msgDir, itemSharedMsgId, editable) of
        ...
        (CIDMBroadcast, SMDSnd, Just itemSharedMId, True) -> do
          ...
          (SndMessage {msgId}, _, _) <- sendDirectContactMessage user ct (XMsgDel itemSharedMId Nothing)
    ...
    CTDirect ->
      withStore (\db -> (,) <$> getContact db user chatId <*> getDirectChatItem db user chatId itemId) >>= \case
        (ct, CChatItem md ci@ChatItem {meta = CIMeta {itemSharedMsgId = Just itemSharedMId}}) -> do
          ...
          (SndMessage {msgId}, _, _) <- sendDirectContactMessage user ct $ XMsgReact itemSharedMId Nothing reaction add
          createdAt <- liftIO getCurrentTime
    ...
    if featureAllowed SCFCalls forUser ct
      then do
        ...
        withChatLock "sendCallInvitation" $ do
          ...
          (msg, _, _) <- sendDirectContactMessage user ct (XCallInv callId invitation)
  ...
  APISendCallOffer contactId WebRTCCallOffer {callType, rtcSession} ->
    ...
    withCurrentCall contactId $ \user ct call@Call {callId, chatItemId, callState} -> case callState of
      CallInvitationReceived {peerCallType, localDhPubKey, sharedKey} -> do
        ...
        (SndMessage {msgId}, _, _) <- sendDirectContactMessage user ct (XCallOffer callId offer)
        withStore' $ \db -> updateDirectChatItemsRead db user contactId $ Just (chatItemId, chatItemId)
  ...
  APISendCallAnswer contactId rtcSession ->
    ...
    withCurrentCall contactId $ \user ct call@Call {callId, chatItemId, callState} -> case callState of
      CallOfferReceived {localCallType, peerCallType, peerCallSession, sharedKey} -> do
        ...
        (SndMessage {msgId}, _, _) <- sendDirectContactMessage user ct (XCallAnswer callId CallAnswer {rtcSession})
        updateDirectChatItemView user ct chatItemId aciContent False $ Just msgId
        pure $ Just call {callState = callState'}
      _ -> throwChatError . CECallState $ callStateTag callState
  ...
  APIEndCall contactId ->
    ...
    withCurrentCall contactId $ \user ct call@Call {callId} -> do
      (SndMessage {msgId}, _, _) <- sendDirectContactMessage user ct (XCallEnd callId)
  ...
  APISyncGroupMemberRatchet gId gMemberId force -> withUser $ \user -> withChatLock "syncGroupMemberRatchet" $ do
    ...
    case memberConnId m of
      Just connId -> do
        cStats@ConnectionStats {ratchetSyncState = rss} <- withAgent $ \a -> synchronizeRatchet a connId pqEncOff force
  ...
  APIAddContact userId incognito -> withUserId userId $ \user -> withChatLock "addContact" . procCmd $ do
    ...
    (connId, cReq) <- withAgent $ \a -> createConnection a (aUserId user) True SCMInvitation Nothing (CR.IKNoPQ $ toPQE enablePQ) subMode
    conn <- withStore' $ \db -> createDirectConnection db user connId cReq ConnNew incognitoProfile subMode enablePQ
  ...
  APIConnect userId incognito (Just (ACR SCMInvitation cReq)) -> withUserId userId $ \user -> withChatLock "connect" . procCmd $ do
    ...
    enablePQ <- readTVarIO =<< asks pqExperimentalEnabled
    connId <- withAgent $ \a -> joinConnection a (aUserId user) True cReq dm (toPQE enablePQ) subMode
  ...
  APICreateMyAddress userId -> withUserId userId $ \user -> withChatLock "createMyAddress" . procCmd $ do
    subMode <- chatReadVar subscriptionMode
    (connId, cReq) <- withAgent $ \a -> createConnection a (aUserId user) True SCMContact Nothing pqInitKeysPQOff subMode
  ...
  SendMessageBroadcast msg -> withUser $ \user -> do
    ...
    where
      ...
      sendToContact user ct = do
        (sndMsg, _, _) <- sendDirectContactMessage user ct (XMsgNew $ MCSimple (extMsgContent mc Nothing))
  ...
  APIAddMember groupId contactId memRole -> withUser $ \user -> withChatLock "addMember" $ do
    ...
    case contactMember contact members of
      Nothing -> do
        gVar <- asks random
        subMode <- chatReadVar subscriptionMode
        (agentConnId, cReq) <- withAgent $ \a -> createConnection a (aUserId user) True SCMInvitation Nothing pqInitKeysPQOff subMode
    ...
    withChatLock "joinGroup" . procCmd $ do
      ...
      case activeConn of
        Just Connection {peerChatVRange} -> do
          ...
          agentConnId <- withAgent $ \a -> joinConnection a (aUserId user) True connRequest dm pqEncOff subMode
          withStore' $ \db -> do
  ...
  APICreateGroupLink groupId mRole -> withUser $ \user -> withChatLock "createGroupLink" $ do
    ...
    (connId, cReq) <- withAgent $ \a -> createConnection a (aUserId user) True SCMContact (Just crClientData) pqInitKeysPQOff subMode
  ...
  APICreateMemberContact gId gMemberId -> withUser $ \user -> do
    ...
    case memberConn m of
      Just mConn@Connection {peerChatVRange} -> do
        ...
        subMode <- chatReadVar subscriptionMode
        (connId, cReq) <- withAgent $ \a -> createConnection a (aUserId user) True SCMInvitation Nothing pqInitKeysPQOff subMode
  ...
  APISendMemberContactInvitation contactId msgContent_ -> withUser $ \user -> do
    ...
    case memberConn m of
      Just mConn -> do
        ...
        (sndMsg, _, _) <- sendDirectMessage mConn pqEncOff msg $ GroupId groupId
  ...
  where
    ...
    sendGrpInvitation user ct@Contact {localDisplayName} gInfo@GroupInfo {groupId, groupProfile, membership} GroupMember {groupMemberId, memberId, memberRole = memRole} cReq = do
      ...
      (msg, _, _) <- sendDirectContactMessage user ct $ XGrpInv groupInv
    ...
    _ -> do
      ...
      case (chatRef, grpMemberId) of
        ...
        (ChatRef CTGroup groupId, Just memId) -> do
          ...
          case activeConn of
            Just conn -> do
              acceptFile CFCreateConnFileInvGroup $ \msg -> void $ sendDirectMessage conn pqEncOff msg $ GroupId groupId
...
acceptContactRequest user UserContactRequest {agentInvitationId = AgentInvId invId, cReqChatVRange, localDisplayName = cName, profileId, profile = cp, userContactLinkId, xContactId} incognitoProfile contactUsed = do
  ...
  dm <- directMessage $ XInfo profileToSend
  enablePQ <- readTVarIO =<< asks pqExperimentalEnabled
  acId <- withAgent $ \a -> acceptContact a True invId dm (toPQE enablePQ) subMode
...
acceptContactRequestAsync :: ChatMonad m => User -> UserContactRequest -> Maybe IncognitoProfile -> Bool -> PQFlag -> m Contact
acceptContactRequestAsync user UserContactRequest {agentInvitationId = AgentInvId invId, cReqChatVRange, localDisplayName = cName, profileId, profile = p, userContactLinkId, xContactId} incognitoProfile contactUsed pqEnabled = do
  subMode <- chatReadVar subscriptionMode
  let profileToSend = profileToSendOnAccept user incognitoProfile False
  (cmdId, acId) <- agentAcceptContactAsync user True invId (XInfo profileToSend) subMode (toPQE pqEnabled)
  withStore' $ \db -> do
    ct@Contact {activeConn} <- createAcceptedContact db user acId (fromJVersionRange cReqChatVRange) cName profileId p userContactLinkId xContactId incognitoProfile subMode pqEnabled contactUsed
...
acceptGroupJoinRequestAsync
  ...
  incognitoProfile = do
    ...
    subMode <- chatReadVar subscriptionMode
    connIds <- agentAcceptContactAsync user True invId msg subMode (toPQE False)
    withStore $ \db -> do
  ...
  where
    ...
    process user = do
                    ...
                    (_, _, SMDSnd, GroupChat g@GroupInfo {groupId}) -> do
                      ...
                      where
                        ...
                        sendToMember (rfd, (conn, sft)) =
                          void $ sendFileDescription sft rfd sharedMsgId $ \msg' -> sendDirectMessage conn pqEncOff msg' $ GroupId groupId
                    _ -> pure ()
                _ -> pure () -- TODO error?
        SFERR e
      ...
      where
        ...
        sendFileDescription :: SndFileTransfer -> ValidFileDescription 'FRecipient -> SharedMsgId -> (ChatMsgEvent 'Json -> m (SndMessage, Int64, CR.PQEncryption)) -> m Int64
        sendFileDescription sft rfd msgId sendMsg = do
          ...
          where
            ...
            loopSend (fileDescr :| fds) = do
              (_, msgDeliveryId, _) <- sendMsg $ XMsgFileDescr {msgId, fileDescr}
...
processAgentMessageConn vr user@User {userId} corrId agentConnId agentMessage = do
  ...
  where
    ...
    processDirectMessage agentMsg connEntity conn@Connection {connId, peerChatVRange, viaUserContactLink, customUserProfileId, connectionCode} = \case
      ...
      Just ct@Contact {contactId} -> case agentMsg of
        ...
        MSG msgMeta _msgFlags msgBody -> do
          let MsgMeta {pqEncryption = CR.PQEncryption pqRcvEnabled} = msgMeta
          (ct', conn') <- updateContactPQRcv user ct conn pqRcvEnabled
        ...
        CON (CR.PQEncryption pqEnabled) ->
          withStore' (\db -> getViaGroupMember db vr user ct) >>= \case
            Nothing -> do
              withStore' $ \db -> updateConnPQEnabledCON db connId pqEnabled
              let conn' = conn {pqSndEnabled = Just pqEnabled, pqRcvEnabled = Just pqEnabled} :: Connection
              ...
              forM_ viaUserContactLink $ \userContactLinkId -> do
                ...
                forM_ autoAccept $ \(AutoAccept {autoReply = mc_}) ->
                  forM_ mc_ $ \mc -> do
                    (msg, _, _) <- sendDirectContactMessage user ct' (XMsgNew $ MCSimple (extMsgContent mc Nothing))
              ...
              CFCreateConnGrpInv -> do
                ...
                where
                  ...
                  sendGrpInvitation ct GroupMember {memberId, memberRole = memRole} groupLinkId = do
                    ...
                    (_msg, _, _) <- sendDirectContactMessage user ct $ XGrpInv groupInv
          ...
          GCInviteeMember -> do
            ...
            where
              sendXGrpLinkMem = do
                ...
                void $ sendDirectMessage conn pqEncOff (XGrpLinkMem profileToSend) (GroupId groupId)
              ...
              processIntro intro@GroupMemberIntro {introId} = do
                void $ sendDirectMessage conn pqEncOff (memberIntro $ reMember intro) (GroupId groupId)
                ...
                GCPreMember ->
                  forM_ (invitedByGroupMemberId membership) $ \hostId -> do
                    ...
                    forM_ (memberConn host) $ \hostConn ->
                      void $ sendDirectMessage hostConn pqEncOff (XGrpMemCon memberId) (GroupId groupId)
                GCPostMember ->
                  forM_ (invitedByGroupMemberId m) $ \invitingMemberId -> do
                    ...
                    forM_ (memberConn im) $ \imConn ->
                      void $ sendDirectMessage imConn pqEncOff (XGrpMemCon memberId) (GroupId groupId)
                ...
                CFCreateConnFileInvGroup -> case grpMemberId of
                  Just gMemberId -> do
                    ...
                    case activeConn of
                      Just gMemberConn -> do
                        sharedMsgId <- withStore $ \db -> getSharedMsgIdByFileId db userId fileId
                        void $ sendDirectMessage gMemberConn pqEncOff (XFileAcptInv sharedMsgId (Just fileInvConnReq) fileName) $ GroupId groupId
                  ...
                  Just groupId -> do
                    ...
                    if isCompatibleRange chatVRange groupLinkNoContactVRange
                      then do
                        ...
                        ct <- acceptContactRequestAsync user cReq profileMode False False
    ...
    probeMatchingMemberContact m@GroupMember {groupId, activeConn = Just conn} connectedIncognito = do
      ...
      where
        sendProbe :: Probe -> m ()
        sendProbe probe = void $ sendDirectMessage conn pqEncOff (XInfoProbe probe) (GroupId groupId)
      ...
      where
        ...
        sendProbeHash cgm@(COMGroupMember m@GroupMember {groupId, activeConn = Just conn}) =
          when (memberCurrent m) $ do
            void $ sendDirectMessage conn pqEncOff (XInfoProbeCheck probeHash) (GroupId groupId)
            withStore' $ \db -> createSentProbeHash db userId probeId cgm
          ...
          case cgm2 of
            COMContact c2@Contact {profile = p2}
              | memberCurrent m1 && isNothing memberContactId && profilesMatch p1 p2 -> do
                  void $ sendDirectMessage conn pqEncOff (XInfoProbeOk probe) (GroupId groupId)
    ...
    sendXGrpMemInv hostConnId directConnReq XGrpMemIntroCont {groupId, groupMemberId, memberId, groupConnReq} = do
      ...
      void $ sendDirectMessage hostConn pqEncOff msg (GroupId groupId)
...
createContactPQSndItem user ct conn@Connection {pqSndEnabled} pqSndEnabled' =
  ...
  flip catchChatError (const $ pure (ct, conn)) $ case (pqSndEnabled, pqSndEnabled') of
    ...
    (Just b, b')
      | b' /= b -> createPQItem $ CISndConnEvent (SCEPQEnabled pqSndEnabled')
      | otherwise -> pure (ct, conn)
  where
    createPQItem ciContent = do
      let cpqe = contactPQEnabled ct
          conn' = conn {pqSndEnabled = Just pqSndEnabled'} :: Connection
          ct' = ct {activeConn = Just conn'} :: Contact
          cpqe' = contactPQEnabled ct'
      when (cpqe' /= cpqe) $ do
        createInternalChatItem user (CDDirectSnd ct') ciContent Nothing
        toView $ CRContactPQEnabled user ct' pqSndEnabled'
      pure (ct', conn')

updateContactPQRcv :: ChatMonad m => User -> Contact -> Connection -> PQFlag -> m (Contact, Connection)
updateContactPQRcv user ct conn@Connection {connId, pqRcvEnabled} pqRcvEnabled' =
  flip catchChatError (const $ pure (ct, conn)) $ case (pqRcvEnabled, pqRcvEnabled') of
    (Nothing, False) -> pure (ct, conn)
    (Nothing, True) -> updatePQ $ CIRcvDirectE2EEInfo (E2EEInfo pqRcvEnabled')
    (Just b, b')
      | b' /= b -> updatePQ $ CIRcvConnEvent (RCEPQEnabled pqRcvEnabled')
      | otherwise -> pure (ct, conn)
  where
    updatePQ ciContent = do
      withStore' $ \db -> updateConnPQRcvEnabled db connId pqRcvEnabled'
      let cpqe = contactPQEnabled ct
          conn' = conn {pqRcvEnabled = Just pqRcvEnabled'} :: Connection
          ct' = ct {activeConn = Just conn'} :: Contact
          cpqe' = contactPQEnabled ct'
      when (cpqe' /= cpqe) $ do
...
sendDirectFileInline :: ChatMonad m => User -> Contact -> FileTransferMeta -> SharedMsgId -> m ()
...
sendMemberFileInline m@GroupMember {groupId} conn ft sharedMsgId = do
  msgDeliveryId <- sendFileInline_ ft sharedMsgId $ \msg -> sendDirectMessage conn pqEncOff msg $ GroupId groupId
  withStore' $ \db -> updateSndGroupFTDelivery db m conn ft msgDeliveryId

sendFileInline_ :: ChatMonad m => FileTransferMeta -> SharedMsgId -> (ChatMsgEvent 'Binary -> m (SndMessage, Int64, CR.PQEncryption)) -> m Int64
sendFileInline_ FileTransferMeta {filePath, chunkSize} sharedMsgId sendMsg =
  ...
  where
    sendChunks chunkNo bytes = do
      ...
      (_, msgDeliveryId, _) <- sendMsg $ BFileChunk sharedMsgId $ FileChunk chunkNo chunk
      if B.null rest
    ...
    cancel' = do
      ...
      when sendCancel $ case fileInline of
        Just _ -> do
          ...
          void . sendDirectMessage conn pqEncOff (BFileChunk sharedMsgId FileChunkCancel) $ ConnectionId connId
...
sendDirectContactMessage :: (MsgEncodingI e, ChatMonad m) => User -> Contact -> ChatMsgEvent e -> m (SndMessage, Int64, CR.PQEncryption)
sendDirectContactMessage user ct chatMsgEvent = do
  ...
  let (_, _, CR.PQEncryption pqEnabled') = r
...
pqEncOff = toPQE False

toPQE :: Bool -> CR.PQEncryption
toPQE = CR.PQEncryption

-- TODO remove in 5.7 (used for groups)
pqInitKeysPQOff :: CR.InitialKeys
pqInitKeysPQOff = CR.IKNoPQ pqEncOff
...
sendGroupMemberMessage user m@GroupMember {groupMemberId} chatMsgEvent groupId introId_ postDeliver = do
  ...
  where
    ...
    messageMember SndMessage {msgId, msgBody} = forM_ (memberSendAction chatMsgEvent [m] m) $ \case
      MSASend conn -> deliverMessage conn pqEncOff (toCMEventTag chatMsgEvent) msgBody msgId >> postDeliver
...
sendPendingGroupMessages user GroupMember {groupMemberId, localDisplayName} conn = do
  ...
  where
    processPendingMessage PendingGroupMessage {msgId, cmEventTag = ACMEventTag _ tag, msgBody, introId_} = do
      void $ deliverMessage conn pqEncOff tag msgBody msgId
      ...
      `catchChatError` \e -> case e of
        ChatErrorStore (SEDuplicateGroupMessage _ _ _ (Just forwardedByGroupMemberId)) -> do
          ...
          forM_ (memberConn fm) $ \fmConn ->
            void $ sendDirectMessage fmConn pqEncOff (XGrpMemCon amMemId) (GroupId groupId)
          throwError e
        _ -> throwError e
  pure (am', conn', msg)
      ...
      ChatErrorStore (SEDuplicateGroupMessage _ _ (Just authorGroupMemberId) Nothing) -> do
        ...
        if sameMemberId refMemberId am
          then forM_ (memberConn forwardingMember) $ \fmConn ->
            void $ sendDirectMessage fmConn pqEncOff (XGrpMemCon amMemberId) (GroupId groupId)
...
createAgentConnectionAsync user cmdFunction enableNtfs cMode subMode = do
  ...
  connId <- withAgent $ \a -> createConnectionAsync a (aUserId user) (aCorrId cmdId) enableNtfs cMode pqInitKeysPQOff subMode
  pure (cmdId, connId)
...
joinAgentConnectionAsync user enableNtfs cReqUri cInfo subMode = do
  ...
  connId <- withAgent $ \a -> joinConnectionAsync a (aUserId user) (aCorrId cmdId) enableNtfs cReqUri cInfo pqEncOff subMode
  pure (cmdId, connId)
