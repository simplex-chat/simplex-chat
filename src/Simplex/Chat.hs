...
processChatCommand' vr = \case
  ...
  APISyncGroupMemberRatchet gId gMemberId force -> withUser $ \user -> withChatLock "syncGroupMemberRatchet" $ do
    ...
    case memberConnId m of
      Just connId -> do
        cStats@ConnectionStats {ratchetSyncState = rss} <- withAgent $ \a -> synchronizeRatchet a connId CR.PQEncOff force
  ...
  APIAddContact userId incognito -> withUserId userId $ \user -> withChatLock "addContact" . procCmd $ do
    ...
    (connId, cReq) <- withAgent $ \a -> createConnection a (aUserId user) True SCMInvitation Nothing (CR.IKNoPQ $ CR.PQEncryption enablePQ) subMode
  ...
  APIConnect userId incognito (Just (ACR SCMInvitation cReq@(CRInvitationUri ConnReqUriData {crAgentVRange} _))) -> withUserId userId $ \user -> withChatLock "connect" . procCmd $ do
    subMode <- chatReadVar subscriptionMode
    -- [incognito] generate profile to send
    incognitoProfile <- if incognito then Just <$> liftIO generateRandomProfile else pure Nothing
    let profileToSend = userProfileToSend user incognitoProfile Nothing False
    dm <- directMessage crAgentVRange $ XInfo profileToSend
    enablePQ <- readTVarIO =<< asks pqExperimentalEnabled
  ...
  APICreateMyAddress userId -> withUserId userId $ \user -> withChatLock "createMyAddress" . procCmd $ do
    subMode <- chatReadVar subscriptionMode
    (connId, cReq) <- withAgent $ \a -> createConnection a (aUserId user) True SCMContact Nothing CR.IKPQOff subMode
  ...
  APIAddMember groupId contactId memRole -> withUser $ \user -> withChatLock "addMember" $ do
    ...
    case contactMember contact members of
      Nothing -> do
        gVar <- asks random
        subMode <- chatReadVar subscriptionMode
        (agentConnId, cReq) <- withAgent $ \a -> createConnection a (aUserId user) True SCMInvitation Nothing CR.IKPQOff subMode
    ...
    withChatLock "joinGroup" . procCmd $ do
      ...
      case activeConn of
        Just Connection {peerChatVRange} -> do
          subMode <- chatReadVar subscriptionMode
          dm <- directMessage (fromJVersionRange peerChatVRange) $ XGrpAcpt membershipMemId
  ...
  APICreateGroupLink groupId mRole -> withUser $ \user -> withChatLock "createGroupLink" $ do
    ...
    (connId, cReq) <- withAgent $ \a -> createConnection a (aUserId user) True SCMContact (Just crClientData) CR.IKPQOff subMode
  ...
  APICreateMemberContact gId gMemberId -> withUser $ \user -> do
    ...
    case memberConn m of
      Just mConn@Connection {peerChatVRange} -> do
        ...
        subMode <- chatReadVar subscriptionMode
        (connId, cReq) <- withAgent $ \a -> createConnection a (aUserId user) True SCMInvitation Nothing CR.IKPQOff subMode
  ...
  APISendMemberContactInvitation contactId msgContent_ -> withUser $ \user -> do
    ...
    case memberConn m of
      Just mConn -> do
        ...
        (sndMsg, _, _) <- sendDirectMessage mConn CR.PQEncOff msg $ GroupId groupId
  ...
  where
    ...
    requestContact user incognito cReq@(CRContactUri ConnReqUriData {crAgentVRange}) xContactId inGroup enablePQ = do
      -- [incognito] generate profile to send
      incognitoProfile <- if incognito then Just <$> liftIO generateRandomProfile else pure Nothing
      let profileToSend = userProfileToSend user incognitoProfile Nothing inGroup
      dm <- directMessage crAgentVRange (XContact profileToSend $ Just xContactId)
      subMode <- chatReadVar subscriptionMode
      connId <- withAgent $ \a -> joinConnection a (aUserId user) True cReq dm (CR.PQEncryption enablePQ) subMode
...
acceptFileReceive user@User {userId} RcvFileTransfer {fileId, xftpRcvFile, fileInvitation = FileInvitation {fileName = fName, fileConnReq, fileInline, fileSize}, fileStatus, grpMemberId, cryptoArgs} rcvInline_ filePath_ = do
  ...
  case (xftpRcvFile, fileConnReq) of
    ...
    (Nothing, Just connReq@(CRInvitationUri ConnReqUriData {crAgentVRange} _)) -> do
      subMode <- chatReadVar subscriptionMode
      dm <- directMessage crAgentVRange $ XFileAcpt fName
    ...
    _ -> do
      ...
      case (chatRef, grpMemberId) of
        ...
        (ChatRef CTGroup groupId, Just memId) -> do
          ...
          case activeConn of
            Just conn -> do
              acceptFile CFCreateConnFileInvGroup $ \msg -> void $ sendDirectMessage conn CR.PQEncOff msg $ GroupId groupId
...
acceptContactRequest user UserContactRequest {agentInvitationId = AgentInvId invId, cReqChatVRange, localDisplayName = cName, profileId, profile = cp, userContactLinkId, xContactId} incognitoProfile contactUsed = do
  ...
  dm <- directMessage (fromJVersionRange cReqChatVRange) $ XInfo profileToSend
  enablePQ <- readTVarIO =<< asks pqExperimentalEnabled
...
acceptContactRequestAsync user UserContactRequest {agentInvitationId = AgentInvId invId, cReqChatVRange, localDisplayName = cName, profileId, profile = p, userContactLinkId, xContactId} incognitoProfile contactUsed pqEnabled = do
  subMode <- chatReadVar subscriptionMode
  let profileToSend = profileToSendOnAccept user incognitoProfile False
  (cmdId, acId) <- agentAcceptContactAsync user True invId (XInfo profileToSend) (fromJVersionRange cReqChatVRange) subMode (CR.PQEncryption pqEnabled)
  withStore' $ \db -> do
...
acceptGroupJoinRequestAsync
  ...
  incognitoProfile = do
    ...
    subMode <- chatReadVar subscriptionMode
    connIds <- agentAcceptContactAsync user True invId msg (fromJVersionRange cReqChatVRange) subMode (CR.PQEncryption False)
    withStore $ \db -> do
                      ...
                      where
                        ...
                        sendToMember (rfd, (conn, sft)) =
                          void $ sendFileDescription sft rfd sharedMsgId $ \msg' -> do
                            (sndMsg, msgDeliveryId, _) <- sendDirectMessage conn CR.PQEncOff msg' $ GroupId groupId
...
processAgentMessageConn vr user@User {userId} corrId agentConnId agentMessage = do
  ...
  where
          ...
          GCInviteeMember -> do
            ...
            where
              sendXGrpLinkMem = do
                ...
                void $ sendDirectMessage conn CR.PQEncOff (XGrpLinkMem profileToSend) (GroupId groupId)
              ...
              processIntro intro@GroupMemberIntro {introId} = do
                void $ sendDirectMessage conn CR.PQEncOff (memberIntro $ reMember intro) (GroupId groupId)
                ...
                GCPreMember ->
                  forM_ (invitedByGroupMemberId membership) $ \hostId -> do
                    ...
                    forM_ (memberConn host) $ \hostConn ->
                      void $ sendDirectMessage hostConn CR.PQEncOff (XGrpMemCon memberId) (GroupId groupId)
                GCPostMember ->
                  forM_ (invitedByGroupMemberId m) $ \invitingMemberId -> do
                    ...
                    forM_ (memberConn im) $ \imConn ->
                      void $ sendDirectMessage imConn CR.PQEncOff (XGrpMemCon memberId) (GroupId groupId)
                ...
                CFCreateConnFileInvGroup -> case grpMemberId of
                  Just gMemberId -> do
                    ...
                    case activeConn of
                      Just gMemberConn -> do
                        sharedMsgId <- withStore $ \db -> getSharedMsgIdByFileId db userId fileId
                        void $ sendDirectMessage gMemberConn CR.PQEncOff (XFileAcptInv sharedMsgId (Just fileInvConnReq) fileName) $ GroupId groupId
    ...
    probeMatchingMemberContact m@GroupMember {groupId, activeConn = Just conn} connectedIncognito = do
      ...
      where
        sendProbe :: Probe -> m ()
        sendProbe probe = void $ sendDirectMessage conn CR.PQEncOff (XInfoProbe probe) (GroupId groupId)
      ...
      where
        ...
        sendProbeHash cgm@(COMGroupMember m@GroupMember {groupId, activeConn = Just conn}) =
          when (memberCurrent m) $ do
            void $ sendDirectMessage conn CR.PQEncOff (XInfoProbeCheck probeHash) (GroupId groupId)
            withStore' $ \db -> createSentProbeHash db userId probeId cgm
    ...
    xFileAcptInv ct sharedMsgId fileConnReq_ fName = do
      ...
      if fName == fileName
        then unless cancelled $ case fileConnReq_ of
          ...
          Just fileConnReq@(CRInvitationUri ConnReqUriData {crAgentVRange} _) -> do
            subMode <- chatReadVar subscriptionMode
            dm <- directMessage crAgentVRange XOk
    ...
    xFileAcptInvGroup GroupInfo {groupId} m@GroupMember {activeConn, memberChatVRange} sharedMsgId fileConnReq_ fName = do
      fileId <- withStore $ \db -> getGroupFileIdBySharedMsgId db userId groupId sharedMsgId
      (AChatItem _ _ _ ci) <- withStore $ \db -> getChatItemByFileId db vr user fileId
      assertSMPAcceptNotProhibited ci
      ...
      if fName == fileName
        then unless cancelled $ case (fileConnReq_, activeConn) of
          (Just fileConnReq, _) -> do
            ...
            -- receiving via a separate connection
            -- [async agent commands] no continuation needed, but command should be asynchronous for stability
            dm <- directMessage (fromJVersionRange memberChatVRange) XOk
            connIds <- joinAgentConnectionAsync user True fileConnReq dm subMode
    ...
    processGroupInvitation ct inv msg msgMeta = do
      ...
      forM_ activeConn $ \Connection {connId, peerChatVRange, customUserProfileId, groupLinkId = groupLinkId'} -> do
        ...
        if sameGroupLinkId groupLinkId groupLinkId'
          then do
            let peerVR = fromJVersionRange peerChatVRange
            subMode <- chatReadVar subscriptionMode
            dm <- directMessage peerVR $ XGrpAcpt membershipMemId
            connIds <- joinAgentConnectionAsync user True connRequest dm subMode
            withStore' $ \db -> do
              setViaGroupLinkHash db groupId connId
              createMemberConnectionAsync db user hostId connIds peerVR subMode
          ...
          case cgm2 of
            COMContact c2@Contact {profile = p2}
              | memberCurrent m1 && isNothing memberContactId && profilesMatch p1 p2 -> do
                  void $ sendDirectMessage conn CR.PQEncOff (XInfoProbeOk probe) (GroupId groupId)
    ...
    sendXGrpMemInv hostConnId directConnReq XGrpMemIntroCont {groupId, groupMemberId, memberId, groupConnReq} = do
      ...
      void $ sendDirectMessage hostConn CR.PQEncOff msg (GroupId groupId)
    ...
    xGrpMemFwd gInfo@GroupInfo {membership, chatSettings} m memInfo@(MemberInfo memId memRole memChatVRange _) introInv@IntroInvitation {groupConnReq, directConnReq} = do
      ...
      dm <- directMessage mcvr $ XGrpMemInfo membershipMemId membershipProfile
    ...
    xGrpDirectInv g m mConn connReq mContent_ msg brokerTs = do
      ...
      where
        ...
        joinConn subMode = do
          ...
          dm <- directMessage (fromJVersionRange $ memberChatVRange m) $ XInfo p
...
sendFileChunkNo ft@SndFileTransfer {agentConnId = AgentConnId acId} chunkNo = do
  ...
  (msgId, _) <- withAgent $ \a -> sendMessage a acId CR.PQEncOff SMP.noMsgFlags $ smpEncode FileChunk {chunkNo, chunkBytes}
    ...
    cancel' = do
      ...
      when sendCancel $ case fileInline of
        Just _ -> do
          ...
          void . sendDirectMessage conn CR.PQEncOff (BFileChunk sharedMsgId FileChunkCancel) $ ConnectionId connId
        _ -> withAgent $ \a -> void . sendMessage a acId CR.PQEncOff SMP.noMsgFlags $ smpEncode FileChunkCancel
      pure fileConnId
...
sendGroupMemberMessage user m@GroupMember {groupMemberId} chatMsgEvent groupId introId_ postDeliver = do
  ...
  where
    ...
    messageMember SndMessage {msgId, msgBody} = forM_ (memberSendAction chatMsgEvent [m] m) $ \case
      MSASend conn -> deliverMessage conn CR.PQEncOff (toCMEventTag chatMsgEvent) msgBody msgId >> postDeliver
...
sendPendingGroupMessages user GroupMember {groupMemberId, localDisplayName} conn = do
  ...
  where
    processPendingMessage PendingGroupMessage {msgId, cmEventTag = ACMEventTag _ tag, msgBody, introId_} = do
      void $ deliverMessage conn CR.PQEncOff tag msgBody msgId
      ...
      `catchChatError` \e -> case e of
        ChatErrorStore (SEDuplicateGroupMessage _ _ _ (Just forwardedByGroupMemberId)) -> do
          ...
          forM_ (memberConn fm) $ \fmConn ->
            void $ sendDirectMessage fmConn CR.PQEncOff (XGrpMemCon amMemId) (GroupId groupId)
          throwError e
        _ -> throwError e
  pure (am', conn', msg)
      ...
      ChatErrorStore (SEDuplicateGroupMessage _ _ (Just authorGroupMemberId) Nothing) -> do
        ...
        if sameMemberId refMemberId am
          then forM_ (memberConn forwardingMember) $ \fmConn ->
            void $ sendDirectMessage fmConn CR.PQEncOff (XGrpMemCon amMemberId) (GroupId groupId)
...
createAgentConnectionAsync user cmdFunction enableNtfs cMode subMode = do
  ...
  connId <- withAgent $ \a -> createConnectionAsync a (aUserId user) (aCorrId cmdId) enableNtfs cMode CR.IKPQOff subMode
  pure (cmdId, connId)
...
joinAgentConnectionAsync user enableNtfs cReqUri cInfo subMode = do
  ...
  connId <- withAgent $ \a -> joinConnectionAsync a (aUserId user) (aCorrId cmdId) enableNtfs cReqUri cInfo CR.PQEncOff subMode
  pure (cmdId, connId)

allowAgentConnectionAsync :: (MsgEncodingI e, ChatMonad m) => User -> Connection -> ConfirmationId -> ChatMsgEvent e -> m ()
allowAgentConnectionAsync user conn@Connection {connId, peerChatVRange} confId msg = do
  cmdId <- withStore' $ \db -> createCommand db user (Just connId) CFAllowConn
  dm <- directMessage (fromJVersionRange peerChatVRange) msg
  withAgent $ \a -> allowConnectionAsync a (aCorrId cmdId) (aConnId conn) confId dm
  withStore' $ \db -> updateConnectionStatus db conn ConnAccepted

agentAcceptContactAsync :: (MsgEncodingI e, ChatMonad m) => User -> Bool -> InvitationId -> ChatMsgEvent e -> VersionRange -> SubscriptionMode -> CR.PQEncryption -> m (CommandId, ConnId)
agentAcceptContactAsync user enableNtfs invId msg peerVR subMode pqEnc = do
  cmdId <- withStore' $ \db -> createCommand db user Nothing CFAcceptContact
  dm <- directMessage peerVR msg
