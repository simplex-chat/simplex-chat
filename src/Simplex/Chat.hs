...
processChatCommand = \case
  ...
  ReceiveFile fileId encrypted rcvInline_ filePath_ -> withUser $ \_ -> do
    cfArgs <- liftIO $ if encrypted then Just <$> CF.randomArgs else pure Nothing
    withChatLock "receiveFile" . procCmd $ do
      (user, ft) <- withStore $ \db -> do
        liftIO $ mapM_ (setFileCryptoArgs db fileId) cfArgs
        getRcvFileTransferById db fileId
