...
getUsersInfo db = do
  ...
  where
    ...
    getUserInfo user@User {userId} = do
      unreadCount <- fromMaybe 0 <$> maybeFirstRow fromOnly (DB.query db "SELECT COUNT(1) FROM chat_items WHERE user_id = ? AND item_status = ? GROUP BY user_id" (userId, CISRcvNew))
