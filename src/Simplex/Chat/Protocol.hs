...
data ChatMsgEvent (e :: MsgEncoding) where
  ...
  XGrpMemBlock :: MemberId -> Bool -> ChatMsgEvent 'Json
...
toCMEventTag msg = case msg of
  ...
  XGrpMemBlock _ _ -> XGrpMemBlock_
...
appJsonToCM AppMessageJson {v, msgId, event, params} = do
  ...
  where
    ...
    msg = \case
      ...
      XGrpMemBlock_ -> XGrpMemBlock <$> p "memberId" <*> p "blocked"
...
chatToAppMessage ChatMessage {chatVRange, msgId, chatMsgEvent} = case encoding @e of
  ...
  where
    ...
    params = \case
      ...
      XGrpMemBlock memId blocked -> o ["memberId" .= memId, "blocked" .= blocked]
