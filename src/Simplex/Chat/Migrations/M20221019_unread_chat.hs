...
ALTER TABLE contacts ADD COLUMN unread_chat INTEGER CHECK (unread_chat NOT NULL);
UPDATE contacts SET unread_chat = 0;
ALTER TABLE groups ADD COLUMN unread_chat INTEGER CHECK (unread_chat NOT NULL);
