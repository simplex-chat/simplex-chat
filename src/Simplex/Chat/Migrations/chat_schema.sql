...
CREATE TABLE contacts(
  ...
  send_rcpts INTEGER,
  member_contact_member_id INTEGER
  REFERENCES group_members(group_member_id) ON DELETE SET NULL,
  member_contact_xgrpdirectinv_sent INTEGER NOT NULL DEFAULT 0,
  FOREIGN KEY(user_id, local_display_name)
  REFERENCES display_names(user_id, local_display_name)
  ON DELETE CASCADE
  ON UPDATE CASCADE,
...
CREATE INDEX idx_contacts_member_contact_member_id ON contacts(
  member_contact_member_id
);
