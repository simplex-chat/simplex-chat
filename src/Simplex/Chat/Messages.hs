...
instance MsgDirectionI d => FromJSON (ChatItem c d) where
...
instance FromJSON (CIDirection c d) where
  parseJSON v = (\ACIDirection _ _ _ dir -> checkDirection dir) <$?> J.parseJSON v
...
instance FromJSON (CIQuote c) where
  parseJSON = J.genericParseJSON J.defaultOptions {J.omitNothingFields = True}
...
data ACIQDirection = forall c . ACIQDirection (SChatType c) (CIQDirection c)
