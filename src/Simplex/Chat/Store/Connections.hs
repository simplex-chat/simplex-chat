...
getConnectionEntity db vr user@User {userId, userContactId} agentConnId = do
  ...
  where
        ...
        DB.query
          ...
          [sql|
            SELECT
              ...
              c.contact_group_member_id, c.contact_grp_inv_sent, c.contact_grp_inv_link, c.contact_grp_inv_from_group_id, c.contact_grp_inv_from_group_member_id, c.contact_grp_inv_from_member_conn_id, c.contact_grp_inv_started_connection,
    ...
    toContact' contactId conn chatTags ((profileId, localDisplayName, viaGroup, displayName, fullName, shortDescr, image, contactLink, localAlias, BI contactUsed, contactStatus) :. (enableNtfs_, sendRcpts, BI favorite, preferences, userPreferences, createdAt, updatedAt, chatTs) :. preparedContactRow :. (contactRequestId, contactGroupMemberId, BI contactGrpInvSent) :. contactGrpInvRow :. (uiThemes, BI chatDeleted, customData, chatItemTTL)) =
      let profile = LocalProfile {profileId, displayName, fullName, shortDescr, image, contactLink, preferences, localAlias}
          ...
          contactGrpInv = toContactGroupInv contactGrpInvRow
