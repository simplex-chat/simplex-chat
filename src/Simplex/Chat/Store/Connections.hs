...
getConnectionEntity db user@User {userId, userContactId} agentConnId = do
  ...
  where
        ...
        <$> DB.query
          ...
          [sql|
            SELECT
              ...
              p.preferences, c.user_preferences, c.created_at, c.updated_at, c.chat_ts, c.member_contact_member_id, c.member_contact_xgrpdirectinv_sent
            FROM contacts c
    ...
    toContact' contactId activeConn [(profileId, localDisplayName, displayName, fullName, image, contactLink, localAlias, viaGroup, contactUsed) :. (enableNtfs_, sendRcpts, favorite, preferences, userPreferences, createdAt, updatedAt, chatTs, memberContactMemberId, memberContactXGrpDirectInvSent)] =
