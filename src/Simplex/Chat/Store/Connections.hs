...
getConnectionEntity db vr user@User {userId, userContactId} agentConnId = do
  ...
  where
        ...
        DB.query
          ...
          [sql|
            SELECT
              ...
              c.ui_themes, c.chat_deleted, c.custom_field1, c.custom_data, c.chat_item_ttl
            FROM contacts c
    ...
    toContact' contactId conn chatTags ((profileId, localDisplayName, viaGroup, displayName, fullName, shortDescr, image, contactLink, peerType, localAlias, BI contactUsed, contactStatus) :. (enableNtfs_, sendRcpts, BI favorite, preferences, userPreferences, createdAt, updatedAt, chatTs) :. preparedContactRow :. (contactRequestId, contactGroupMemberId, BI contactGrpInvSent) :. groupDirectInvRow :. (uiThemes, BI chatDeleted, customField1, customData, chatItemTTL)) =
