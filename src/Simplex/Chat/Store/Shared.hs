...
groupMemberQuery =
  [sql|
    SELECT
      m.group_member_id, m.group_id, m.sequential_id, m.member_id, m.peer_chat_min_version, m.peer_chat_max_version, m.member_role, m.member_category, m.member_status, m.show_messages, m.member_restriction,
...
groupInfoQueryFields =
  [sql|
    SELECT
      ...
      mu.group_member_id, mu.group_id, mu.sequential_id, mu.member_id, mu.peer_chat_min_version, mu.peer_chat_max_version, mu.member_role, mu.member_category,
