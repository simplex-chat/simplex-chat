...
keySendStatusText :: KeySendStatus -> Maybe Text
keySendStatusText = \case
  KSSent -> Just "sent"
  KSFailed -> Just "failed"
  KSError e -> Just $ "error " <> e
  KSAttempts _ -> Nothing

toKeySendStatus :: Maybe Text -> Int64 -> KeySendStatus
toKeySendStatus status attempts = case status of
  Nothing -> KSAttempts $ fromIntegral attempts
  Just "sent" -> KSSent
  Just "failed" -> KSFailed
  Just s -> case T.stripPrefix "error " s of
    Just e -> KSError e
    Nothing -> KSError s

toGroupMember :: UTCTime -> Int64 -> GroupMemberRow -> GroupMember
toGroupMember now userContactId ((groupMemberId, groupId, indexInGroup, memberId, minVer, maxVer, memberRole, memberCategory, memberStatus, BI showMessages, memberRestriction_) :. (invitedById, invitedByGroupMemberId, localDisplayName, memberContactId, memberContactProfileId) :. profileRow :. (createdAt, updatedAt) :. (supportChatTs_, supportChatUnread, supportChatMemberAttention, supportChatMentions, supportChatLastMsgFromMemberTs, memberPubKey, keyStatus_, keyAttempts, relayLink, memberCode_, memberCodeVerifiedAt_)) =
  let memberProfile = rowToLocalProfile now profileRow
      ...
      activeConn = Nothing
      userMemberKeyStatus = toKeySendStatus keyStatus_ keyAttempts
...
groupMemberQuery =
  [sql|
    SELECT
      ...
      m.support_chat_ts, m.support_chat_items_unread, m.support_chat_items_member_attention, m.support_chat_items_mentions, m.support_chat_last_msg_from_member_ts, m.member_pub_key, m.user_member_key_status, m.user_member_key_attempts, m.relay_link, m.member_security_code, m.member_security_code_verified_at,
