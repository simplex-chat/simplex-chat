...
processChatCommand vr nm = \case
  ...
  APIForwardChatItems toChat@(ChatRef toCType toChatId toScope) fromChat@(ChatRef fromCType fromChatId _fromScope) itemIds itemTTL _sendAsGroup -> withUser $ \user -> case toCType of
    CTDirect -> do
