...
cleanupManager = do
  ...
  where
    ...
    cleanupUser cleanupInterval stepDelay user = do
      ...
      liftIO $ threadDelay' stepDelay
      cleanupUnusedContacts user `catchChatError` (toView . CRChatError (Just user))
      liftIO $ threadDelay' stepDelay
    ...
    cleanupUnusedContacts user@User {userId} = do
      vr <- chatVersionRange
      contacts <- withStore' $ \db -> getUnusedContacts db vr user
      (errs1, (errs2, connIds)) <- lift $ second unzip . partitionEithers <$> withStoreBatch (\db -> map (deleteUnusedContact db vr) contacts)
      let errs = errs1 <> mapMaybe (fmap ChatErrorStore) errs2
      unless (null errs) $ toView $ CRChatErrors (Just user) errs
      deleteAgentConnectionsAsync user $ concat connIds
      where
        deleteUnusedContact :: DB.Connection -> VersionRangeChat -> Contact -> IO (Either ChatError (Maybe StoreError, [ConnId]))
        deleteUnusedContact db vr ct = runExceptT . withExceptT ChatErrorStore $ do
