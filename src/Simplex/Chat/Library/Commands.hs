...
processChatCommand cxt nm = \case
  ...
  APITestProtoServer userId srv@(AProtoServerWithAuth _ server) -> withUserId userId $ \user ->
    lift $ (\case Left err -> CRServerTestResult user srv (Just err) Nothing; Right info -> CRServerTestResult user srv Nothing info) <$> withAgent' (\a -> testProtocolServer a nm (aUserId user) server)
