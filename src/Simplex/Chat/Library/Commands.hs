...
processChatCommand vr nm = \case
  ...
  where
    ...
    forwardMsgContent ChatItem {content = CISndMsgContent fmc} = pure $ Just $ dropOwnerSig fmc
    forwardMsgContent ChatItem {content = CIRcvMsgContent fmc} = pure $ Just $ dropOwnerSig fmc
