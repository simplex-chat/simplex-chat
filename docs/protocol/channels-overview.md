...
- Re-elevate a **removed** member by replay. A privileged removal (`x.grp.mem.del`) now carries the roster version and advances the subscriber the same way `x.grp.mem.role` does: after `promote cath (Vn)` then `remove cath (Vn+1)`, a subscriber that processed the removal is at `Vn+1`, so a replayed `x.grp.mem.role(cath -> admin, Vn)` is rejected (`Vn < Vn+1`). This closes the gap that was previously an accepted limitation (when removals were version-less). The roster version is now contiguous across exactly the roster-affecting changes - promotions, demotions, and privileged removals; observer removals don't bump (they don't touch the roster set). The owner's roster blob, served to joiners/resumers, excludes the removed member, so the removal also reaches new joiners.
