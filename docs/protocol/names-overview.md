...
A SimpleX name is a human-readable name, registered on a public blockchain, that resolves to SimpleX links: `#name.simplex` opens a channel (for .simplex namespace, a short variant `#name` also works) and `@name.simplex` connects to a contact or business address. One name can serve both - a business can publish `@name.simplex` for customer conversations and `#name.simplex` for its announcement channel from a single registration. Names support subnames (`support.name.simplex`). A separate test namespace (`.testing`) runs the same system for early adopters before the main namespace launches.
...
Names are for entities that want to be public:

- **Channels.** A channel reachable as `#name` can be easily shared in many ways - conversation, print, social media, and messages. If the channel's link is destroyed or blocked, the owner points the name at a new link and the audience finds it again.
...
Names are opt-in and only make sense for public entities. Private users do not need them. Even when entity uses a name, network cannot track who connects to these names - the communication graph remains private.
...
A record on a public blockchain has neither owner nor operator: only the holder of the registration key can change what a name points to, and no authority exists that can delete it. This mitigates censorship at the levels where links fail:

- **A link is hosted by one SMP router.** That router's operator can delete it, and the link is destroyed for everyone who saved it. A name survives: the owner points it at a new link on a different router, and everyone can connect by the name they already know.
...
- **Put communication on the chain.** The blockchain stores only the mapping from a name to links and optional public profile fields. Messages, membership, and channel content never get to blockchain.
...
Names are lowercase labels of ASCII letters, digits, and single hyphens, forming domains under the `.simplex` top-level name (`privacy.simplex`, `my-channel.simplex`), with `.simplex` implied when omitted for channels: `#privacy` means `#privacy.simplex`. The restricted alphabet is deliberate: visually identical names from mixed scripts (homographs) cannot exist as distinct records, so a name that looks right is right.

A name's on-chain record stores:

- **Channel links and contact links** - each an ordered list, primary first. Multiple links give redundancy across SMP routers: clients try them in order, so the loss of one router does not break the name. This redundandancy is planned in the apps at the same time `.simplex` namespace launches.
- **Optional profile fields** - a display name, website, location.
- **Optional donation addresses** (Monero, Bitcoin, Ethereum, etc.) that apps can show to the channel subscribers.
...
This layer is not specific to names. An anonymous access to blockchain state via independent operators is a network capability that could be used in other contexts in the future.
...
Names bootstrap connections, they are not used to deliver messages. Once connected, a contact or channel subscription is an ordinary SimpleX connection, independent of the name. A name expiring or changing hands affects future connections only, it does not affect existing conversations or subscriptions.
...
3. **No name wrapper, no separate subname ownership.** ENS's wrapper system lets subnames be split off and owned independently, at the cost of a second token standard and a complex permission system. SNS subnames belong to the name and follow it (the DNS model), removing the need for the wrapper.
...
Current implementation uses a web app for registration, which uses a centralized RPC layer. We are planning to add ability to purchase names via the app, via the same decentralized blockchain access that is already used for name resolution.
...
3. **Impersonation resistance.** A name shows as verified only when the on-chain record and the signed claim in the link's profile agree.
...
- See that a client communicates with a names router, which is the usual SMP router.
...
- Register a confusingly similar name. The restricted alphabet keeps such names visibly different - no homographs allowed.
...
2. **No state proofs.** The names router is trusted to report chain state faithfully; responses do not yet include proofs verifiable against a chain header.
...
- **DNS-anchored names.** The client already parses arbitrary domains (`name.example.com`) as web links. In the future, it will be possible to register owned domains in SNS contract to use them as channel and contact names (`#name.example.com` and `@name.example.com`).


## Conclusion

SimpleX names give channels and businesses what public communication needs and links alone cannot provide: an identity that is memorable, that survives the loss of any server, and that no platform or registrar can revoke - while the people who look a name up remain private, because resolution runs through the same identifier-free network as the communication itself. The naming layer keeps the system fully decentralized on-chain, with no indexer or metadata service, and fully private in access, with no RPC observing requests. Public names for the few who want them; no identifiers for everyone else.
