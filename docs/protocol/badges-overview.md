...
The badge service holds the purchase key, the number of months bought, and whatever payment record accompanies the purchase. A card payment is linked to the purchase, as in any other service; a user who wants the payment itself to be private may pay in Monero. In either case the record is held by the service alone, and the service does not learn where the badge is later shown or used, because a proof contains nothing that refers back to the credential.
