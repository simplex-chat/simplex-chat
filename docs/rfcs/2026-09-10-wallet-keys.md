...
/_wallet scan                      walk the accounts of a recovered master,
                                   binding the ones in use to the active profile
...
- **Someone reading the logs.** A remote session logs the whole command line at debug level, so a phrase typed into `create` reaches the log there. Answers are never logged. Narrowing that line is work in the remote session rather than in the wallet.
