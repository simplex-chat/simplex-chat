...
broadcastBot BroadcastBotOpts {publishers, welcomeMessage, prohibitedMessage} _user cc = do
  ...
  race_ (forever $ void getLine) . forever $ do
    ...
    case resp of
            ...
            then do
              sendChatCmd cc "/contacts" >>= \case
                CRContactsList _ cts -> void . forkIO $ do
                  ...
                  sendReply $ "Przekazane do " <> show (length cts') <> " kontakt(y)"
                r -> putStrLn $ "Błąd pobierania listy kontaktów: " <> show r
            else sendReply "!1 Wiadomość nie jest obsługiwana!"
        | otherwise -> do
      ...
      _ -> pure ()
  where
    contactConnected ct = putStrLn $ T.unpack (localDisplayName' ct) <> " podłączony"
