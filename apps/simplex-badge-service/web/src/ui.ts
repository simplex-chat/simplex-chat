...
export interface Shell {
  /** Show the error banner. Nothing on this site fails to a blank screen. */
...
export function toDom(el: El): HTMLElement {
  const node = document.createElement(el.tag)
  for (const name of Object.keys(el.attrs)) node.setAttribute(name, el.attrs[name])
  for (const child of el.children) node.append(childNode(child))
  return node
}

function childNode(child: Child): HTMLElement | string {
