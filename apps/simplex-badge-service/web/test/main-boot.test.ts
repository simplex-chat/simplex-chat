// main.ts runs on import and a process gets one boot, so a second boot URL needs its own file.
...
// A fetch called with an already-aborted signal never fires abort, so the stub must reject it or the request would hold until the test times out.
