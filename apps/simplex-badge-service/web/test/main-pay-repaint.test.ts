// A panel rebuild defeats the disabled attribute, and /api/invoice has no idempotency key, so a second POST is a second charge and a second code.
...
payTest("main: a repaint while the checkout is on the wire does not arm a second Pay", async () => {
  ...
  // No await here, so the POST stays on the wire while Back and Continue rebuild the Pay button without its disabled attribute.
  ...
  // pay() hashes the code before posting, so the count that matters is after both clicks have reached the network.
  await settle();
