...
m20260821_badge_service_web :: Text
...
CREATE TABLE @codes(
  ...
  badge_type TEXT NOT NULL CHECK (badge_type IN ('supporter','legend')),
  months SMALLINT NOT NULL CHECK (months > 0), -- lifetime codes are out of scope, see §6
  batch TEXT NOT NULL,                  -- 'web' for web orders, else an operator batch name
  expires_at TIMESTAMPTZ NOT NULL,             -- redemption deadline -> code_expired
  redeemed_purchase_id BIGINT REFERENCES @badge_purchases,
  redeemed_at TIMESTAMPTZ,
  unredeemed_at TIMESTAMPTZ,                   -- set by B1's unredeemCode; reopens E4's window
...
CREATE TABLE @provider_events(
  ...
  processed_at TIMESTAMPTZ,                    -- NULL means the previous attempt did not complete (E3)
  PRIMARY KEY(provider, event_id)
);
|]

down_m20260821_badge_service_web :: Text
