...
Both placements go through `chatListBanner()` (`views/newchat/OnboardingCards.kt`), which fills the list's single banner slot in priority order — `ChatListBanner.BadgeExpired`, `BadgePitch`, `GetStake` — and records the kind that took it for the session; for the rest of the app session only that kind is shown (while its own conditions hold), so dismissing it never puts another in its place until restart. The one exception is the support-ended alert (`supportEnded()`): whenever it is present it takes the slot over from any kind, and then holds it the same way. The choice waits until `BadgeModel.isCurrent` for the current user, whose state the pitch's conditions read.
