...
The list has a single banner slot. The support-ended alert (`supportEnded()`) is time-sensitive and takes the slot whenever it is present. Otherwise both placements go through `chatListBanner()` (`views/newchat/OnboardingCards.kt`), which picks the first of `ChatListBanner.BadgePitch`, `GetStake` whose conditions hold and records it for the session; for the rest of the app session only that kind is shown (while its own conditions hold), so dismissing it never puts the other in its place until restart. The choice waits until `BadgeModel.isCurrent` for the current user, whose state the pitch's conditions read.
