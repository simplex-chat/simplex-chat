...
func getLinkMetadata(url: URL, cb: @escaping (LinkPreview?) -> Void) {
           ...
           imageProvider.canLoadObject(ofClass: UIImage.self) {
            imageProvider.loadObject(ofClass: UIImage.self){ object, error in
                ...
                if let error = error {
                    ...
                    if let image = resizeAndCompressImage(image: image, maxHeight: 96),
                       let title = metadata.title,
                       let uri = metadata.originalURL {
                        linkPreview = LinkPreview(uri: uri, title: title, image: image)
                    }
                }
                cb(linkPreview)
            }
...
struct ComposeLinkPreviewView: View {
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 8) {
            if let data = Data(base64Encoded: dropImagePrefix(metadata.image)),
            ...
            VStack(alignment: .center, spacing: 0) {
                Text(metadata.title)
                    .lineLimit(1)
                Text(metadata.uri.absoluteString)
                    .font(.caption)
...
struct SmallLinkPreview_Previews: PreviewProvider {
    static var previews: some View {
        ...
        ComposeLinkPreviewView(metadata: preview, cancelPreview: {})
