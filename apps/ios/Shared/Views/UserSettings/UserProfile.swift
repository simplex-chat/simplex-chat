...
struct UserProfile: View {
        ...
        .onChange(of: pickedImage) { image in
            if let image = image,
               let data = resizeToSquare(image, 128).jpegData(compressionQuality: 0.85) {
                profile.image = "data:image/jpg;base64,\(data.base64EncodedString())"
            } else {
                profile.image = nil
            }
        }
    ...
    func saveProfile() {
        Task {
            do {
                try await apiUpdateProfileImage(image: profile.image)
                let newProfile = try await apiUpdateProfile(profile: profile)
...
func resizeToSquare(_ image: UIImage, _ edge: CGFloat) -> UIImage {
    let size = image.size
