...
struct ChatInfoView: View {
    ...
    private func networkStatusRow() -> some View {
        HStack {
            ...
            Image(systemName: "info.circle")
                ...
                .font(.system(size: 14))
            Spacer()
            Text(chatModel.connNetworkStatus(contact.activeConn).statusString)
                .foregroundColor(.secondary)
            serverImage()
        }
    }
    ...
    private func serverImage() -> some View {
        let status = chatModel.connNetworkStatus(contact.activeConn)
    ...
    private func networkStatusAlert() -> Alert {
        Alert(
            ...
            message: Text(chatModel.connNetworkStatus(contact.activeConn).statusExplanation)
        )
    }
