...
struct ChatView: View {
    ...
    private func voiceWithoutFrame(_ ci: ChatItem) -> Bool {
        ci.content.msgContent?.isVoice == true && ci.content.text(false).count == 0 && ci.quotedItem == nil && ci.meta.itemForwarded == nil
    }
    ...
    private struct ChatItemWithMenu: View {
        ...
        private func menu(_ ci: ChatItem, _ range: ClosedRange<Int>?, live: Bool) -> some View {
            if case let .group(gInfo, _) = chat.chatInfo, ci.isReport, ci.meta.itemDeleted == nil {
                ...
                let copyAndShareAllowed = !ci.content.text(false).isEmpty || (ci.content.msgContent?.isImage == true && fileExists)
