...
public struct RelayTestFailure: Decodable, Error {
    ...
    var localizedDescription: String {
        ...
        case .errorAgent(agentError: .SMP(_, .AUTH)):
            return err + " " + NSLocalizedString("Router requires authorization to connect to relay, check password.", comment: "relay test error")
        case .errorAgent(agentError: .BROKER(_, .NETWORK(.unknownCAError))):
            return err + " " + NSLocalizedString("Fingerprint in router address does not match certificate.", comment: "relay test error")
        default:
