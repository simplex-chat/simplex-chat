...
- **Reconnect pairing**: iOS stores the verified controller ID and exact invitation as one `UserDefaults` value. A retained reconnect verifies the saved paired identity automatically and creates fresh TLS and session keys.
- **Reconnect commands**: A connect command starts a cancellable dial and returns its session sequence without waiting for the dial to complete. Internal `/_abort remote ctrl <sessionSeq>` silently detaches the matching controller session in any state; the coordinator uses it for a rejected attempt. `/_drop remote ctrl <sessionCode>` closes only the matching connected transport.
- **Network and retry**: [`NetworkObserver`](../Shared/Model/NetworkObserver.swift#L13) reports path loss immediately. [`RemoteCtrlReconnect`](../Shared/Model/SimpleXAPI.swift#L1773) changes the UI to reconnecting and retries only on Wi-Fi or Ethernet. It tries immediately, then waits 1, 2, 4, 8, and 10 seconds between attempts; later waits stay at 10 seconds. It never retries on cellular. Chat start and app activation resume the same retry intent.
- **Ownership and correlation**: QR, controller-confirmation, and automatic reconnect work uses connection generations and exact session sequences. Late events cannot change another attempt. A late successful request that lost ownership is aborted by its exact sequence. Stop invalidates all request and verification generations before it clears the target.
- **UI lifecycle**: [`ConnectDesktopView`](../Shared/Views/RemoteAccess/ConnectDesktopView.swift#L13) renders reconnecting separately from connected. [`ChatListView`](../Shared/Views/ChatList/ChatListView.swift#L155) retains the remote-access sheet through connecting and verification. [`RemoteCtrlBGKeepAlive`](../Shared/Model/SuspendChat.swift#L375) keeps the background session active and updates the Live Activity to show reconnecting. Explicit Disconnect ends the activity.
- **App lifecycle**: Chat suspension silently detaches the current remote transport and preserves the retry target. Database deletion clears the target. The no-suspend background keepalive shutdown does not run a second suspension.
- **Host lifecycle**: [`startRemoteHost`](../../../src/Simplex/Chat/Remote.hs#L156) keeps one listener and signed invitation across transport loss. Each restored handshake must match the paired mobile identity fingerprint. A transport loss emits `RHSConnecting` with the same host session sequence.
- **iOS to macOS stop**: [`stopRemoteCtrl`](../../../src/Simplex/Chat/Remote.hs#L812) sends the authenticated `remoteCtrlStopped` event before it closes an active transport. The desktop treats that event as terminal and closes the retained listener.
- **macOS to iOS stop**: [`closeRemoteHost`](../../../src/Simplex/Chat/Remote.hs#L342) sends the authenticated encrypted `/_stop remote ctrl` command. The remote command path intercepts it and calls [`stopRemoteCtrlByController`](../../../src/Simplex/Chat/Remote.hs#L829), which emits `controllerStopped`. iOS clears its reconnect target and does not retry or show an error.
- **Offline stop limit**: Explicit stop signals require an active transport. A side that stops while offline clears its local intent but cannot update its peer. The peer retains its listener or retry intent until a local stop, deletion, or app shutdown.
- **Reconnect transport**: Encrypted reverse HTTP over the existing XRCP TLS connection. Reconnect does not use multicast discovery, an extracted IP address, or another fallback.
- **Process lifetime**: The desktop invitation private keys remain in memory. A desktop process restart requires a new QR invitation.
- **Compatibility**: Both peers must support remote protocol version `7.1.0.0` or later.
- **Reconnect source**: [`AppAPITypes.swift`](../Shared/Model/AppAPITypes.swift#L1007), [`ConnectDesktopView.swift`](../Shared/Views/RemoteAccess/ConnectDesktopView.swift#L13), [`Remote.hs`](../../../src/Simplex/Chat/Remote.hs#L156), [`Remote/Types.hs`](../../../src/Simplex/Chat/Remote/Types.hs#L104), and [`Library/Commands.hs`](../../../src/Simplex/Chat/Library/Commands.hs#L3591).
