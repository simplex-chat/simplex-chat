...
The list has a single banner slot. The support-ended alert (`supportEnded`) is time-sensitive and takes the slot whenever it is present. Otherwise both placements go through `ChatListView.chatListBanner`, which picks the first of `ChatListBanner.badgePitch`, `.getStake` whose conditions hold and records it in `ChatListBanner.shownThisSession`; for the rest of the app session only that kind is shown (while its own conditions hold), so dismissing it never puts the other in its place until restart. The choice waits until `BadgeModel` holds the current user's state, which the pitch's conditions read.
