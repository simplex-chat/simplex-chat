...
public struct ThemeModeOverride: Codable, Equatable {
    ...
    public func removeSameColors(_ base: DefaultTheme) -> ThemeModeOverride {
        let c = switch base {
        case DefaultTheme.LIGHT: LightColorPalette
        case DefaultTheme.DARK: DarkColorPalette
        case DefaultTheme.SIMPLEX: SimplexColorPalette
        ...
        return ThemeModeOverride(
            mode: self.mode,
            colors: ThemeColors(
                primary: colors.primary?.colorFromReadableHex() != c.primary ? colors.primary : nil,
                primaryVariant: colors.primaryVariant?.colorFromReadableHex() != c.primaryVariant ? colors.primaryVariant : nil,
                secondary: colors.secondary?.colorFromReadableHex() != c.secondary ? colors.secondary : nil,
                secondaryVariant: colors.secondaryVariant?.colorFromReadableHex() != c.secondaryVariant ? colors.secondaryVariant : nil,
                background: colors.background?.colorFromReadableHex() != c.background ? colors.background : nil,
                surface: colors.surface?.colorFromReadableHex() != c.surface ? colors.surface : nil,
                title: colors.title?.colorFromReadableHex() != ac.title ? colors.title : nil,
                primaryVariant2: colors.primaryVariant2?.colorFromReadableHex() != ac.primaryVariant2 ? colors.primary : nil,
                sentMessage: colors.sentMessage?.colorFromReadableHex() != ac.sentMessage ? colors.sentMessage : nil,
                sentQuote: colors.sentQuote?.colorFromReadableHex() != ac.sentQuote ? colors.sentQuote : nil,
                receivedMessage: colors.receivedMessage?.colorFromReadableHex() != ac.receivedMessage ? colors.receivedMessage : nil,
                receivedQuote: colors.receivedQuote?.colorFromReadableHex() != ac.receivedQuote ? colors.receivedQuote : nil
            ),
            wallpaper: wallpaper
        )
    }
