...
import Simplex.Messaging.Agent.Protocol (AConnectTarget (..), AConnectionLink (..), ACreatedConnLink (..), AgentErrorType (..), ConnectionLink (..), CreatedConnLink (..), SConnectionMode (..), sameConnReqContact, sameShortLinkContact)
    ...
    deGroupUpdated m@GroupMember {memberProfile = LocalProfile {displayName = mName}} fromGroup toGroup = do
      ...
      where
        ...
        publicGroupProfileChange pg@PublicGroupProfile {groupLink} gr byMember n' = do
          ...
          sendChatCmd cc (APIConnectPlan userId (Just (ACTLink link)) True Nothing) >>= \case
      ...
      case groupType of
        ...
        gt -> do
          let link = ACL SCMContact $ CLShort connLink
              ...
              gt' = groupTypeStr gt
          sendChatCmd cc (APIConnectPlan userId (Just (ACTLink link)) True (Just ownerSig)) >>= \case
