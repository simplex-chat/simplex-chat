      ...
      where
        ...
        captchaNotice =
          "Captcha is generated by SimpleX Directory service.\n\n*Send captcha text* to join the group " <> displayName <> "."
            <> if isJust (voiceCaptchaGenerator opts) then "\nSend /audio to switch between text and voice captcha." else ""
          ...
          atomically (TM.lookup gmId $ pendingCaptchas env) >>= \case
            ...
            Just pc@PendingCaptcha {captchaText, sentAt, attempts, captchaMode}
              | isAudioCmd -> case captchaMode of
                  ...
                  CMAudio -> do
                    atomically $ TM.insert gmId pc {captchaMode = CMText} $ pendingCaptchas env
                    let noticeText = "*Send captcha text* to join the group " <> n <> "."
                    mc <- getCaptchaContent (T.unpack captchaText)
                    sendComposedMessages_ cc sendRef [(Just ciId, MCText noticeText), (Nothing, mc)]
