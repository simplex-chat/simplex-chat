...
- Short-circuit on receipt (both `memberApplyRoster` and `relayApplyRoster`) unless `version > max(groups.roster_version, in-flight group_roster_recv.version)` — strictly greater than BOTH the applied and any in-flight version, **before** creating a file. Comparing only against the applied version is a bug (GAP 1): the QCONT re-serve below is unconditional, so the relay can re-forward its still-cached v5 while the member is mid-receiving v6 (applied = 4); v5 > 4 would supersede the in-flight v6, and the arriving v6 chunks then fail the v5 digest → member stuck. Never call `setGroupRosterVersion`/`setCachedGroupRoster` in the header handler (else the genuine blob completes as an equal-version no-op and the receiver is stuck at `vN` with `v(N-1)` data).
...
- One in-flight roster file per group (`group_roster_recv.group_id` PK). A duplicate header (same `shared_msg_id`/version) is idempotent. A version strictly greater than both applied and in-flight supersedes — fully clean up the stale transfer (cleanup below), then create the new.
...
**Supersede / cancel cleanup** spans ALL of: `files`, `rcv_files`, `rcv_file_chunks`, the on-disk file, the `rcvFiles` handle map (`closeFileHandle`), and the `group_roster_recv` row — miss none.
