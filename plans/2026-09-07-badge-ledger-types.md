...
Re-running is what catches over-lapsing: writing off three months when one elapsed adds up against its predecessor, and it is the theft case, emptying `balanceMonths` while leaving `paidThrough` untouched. Two values cannot be re-derived and are bounded instead — a credit's months, of which only the sign is checkable, and `createdAt`, which every recompute is anchored on, held below the client's clock and at or after its predecessor's.
