...
**Compare-and-swap.** "Write this, but only if what is stored is still what I read." The writer passes a fingerprint — a hash of the stored user-data bytes its merge was based on — and the server stores the new data only if the current bytes still hash to that value; otherwise it rejects and stores nothing. A rejection *is* the detection, so no verify read is needed: re-merge and retry. Lossless.

**A rejection returns the current data**, so the retry needs no read: reject → merge → write, two operations rather than three. The rejected writer always wants that data — it cannot re-merge without it — so nothing is sent in vain, and it is the same bytes it would have fetched. Nothing new leaks: `LGET` requires no signature at all for a contact queue (`Server.hs:1256`), so link data is already public to anyone holding the link, and this hands it to an authenticated recipient. The transport already carries payloads of that size — `LNK` (`Protocol.hs:698`) returns a whole `QueueLinkData` today — though a rejection needs only the user data, since the fixed half never changes and the writer already has it. It wants a distinct response rather than an `ErrorType`, which is not shaped for a 13.8 KB payload. Worth deciding now rather than later: adding it afterwards is a second version bump, whereas now it is one field on a response we are already introducing.
...
**Where a writer gets the fingerprint.** Only from bytes it has seen, because it is a hash of ciphertext: from its own successful write (the stored bytes are the bytes it just sent, hashed on the way out), from the reconcile tick's `LGET`, or from a rejection, which returns the current data. **Another owner's write is otherwise opaque** — you cannot re-derive its fingerprint from your local plaintext, since the nonce is fresh on every write. It is persisted next to the link credentials, so a restart does not force a read.

**Precisely: a write is rejected exactly when another owner has written since you last read or wrote.** So owners alternating do pay — the first write after another owner's is rejected, costing one extra round trip, and that owner is warm again afterwards. That is optimistic concurrency working as intended: you pay only when a conflict actually happened, which for human-paced writes means once per alternation.
...
**The fingerprint never reaches chat.** The agent performs the encryption, so it is the only layer that sees the stored bytes; it computes the fingerprint, keeps it with the link credentials it already stores, and refreshes it on every successful write, `LGET` and rejection. Chat calls `setConnShortLink` as it does now and gets back either success or a conflict carrying the current data decrypted; on conflict it re-merges its delta onto that and calls again. Merge policy stays in chat, link crypto stays in the agent.
...
- Compare-and-swap on `LSET` (§4.2): a new command tag carrying an expected fingerprint of the stored user data, and a new `BrokerMsg` response for the rejection carrying the current user data — not an `ErrorType`, which is not shaped for a 13.8 KB payload. `currentSMPClientVersion` 4 → 5. The server check goes beside `lnkId' /= lnkId -> err AUTH` in the `LSET` handler (`Server.hs:1484`); the bytes to hash and to return are already in `queueData qr`, so no storage change. Agent side: keep the fingerprint with the link credentials, refresh it on every successful write, `LGET`, and rejection, and surface a conflict to chat with the current data attached. Fall back to writing blind against servers below v5.
