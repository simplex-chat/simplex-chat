...
**1. Core remembers the failure.** Four new columns on `badge_purchases`. Three are written only by the renewal request path (`requestBadgeIssue`), never by retirement or presentation: `issue_failed_since` (the first failed attempt of the current run), `issue_error_at` and `issue_error` (the last failure: when, and what). A renewal request counts as failed when it ends without a new credential stored: the service refused, the request threw (timeout, network, undecodable or unexpected reply), or the issued credential did not verify — that last case stores the ledger row today and is still recorded as a failure. Nothing clears these columns: a failure is current while `issue_error_at` is later than the newest row in `badge_issuances`, and an older one stays in the row for debugging but counts for nothing. A failure recorded while none is current starts a new run, and sets `issue_failed_since`. The fourth column, `next_wake_at`, is the wake the worker is about to wait for, written at the end of every pass whether it failed or not.

`BadgeState` gains `issueError :: Maybe BadgeIssueError` — `{failedSince, lastAttemptAt, reason, current}`, the last failure recorded, with `current` saying whether it still is one — and `nextWakeAt :: Maybe UTCTime`. `reason` is a sum of the failure kinds below. That is the whole UI contract; nothing else is added to the wire.

**2. Core decides when it is an alert.** A new alert kind, `BAIssueFailed`, derived from stored state like the others. It is raised when `issueError` is current and either
...
`episode` is `issue_failed_since`, so one alert per run of failures; acknowledging it (the existing `/_badge ack`, snooze included) silences that run and a later run alerts again. A successful renewal ends the run, and with it the alert and the Error section; a failure after that is a new run with a new episode.
...
**5. Developer tools** (the Credential section): "Next check" from `nextWakeAt` when present, and whenever `issueError` is present, current or not, "Last attempt" with `lastAttemptAt` and "Last error" with the raw reason (tag plus payload, e.g. `serviceError code_used`). The CLI's `/_badge state` prints the same: `, next check YYYY-MM-DD HH:MM` on the state line, and a second line `renewal failed YYYY-MM-DD HH:MM: <reason as stored>`, with ` (current)` appended while it is.
...
data BadgeIssueError = BadgeIssueError {failedSince :: UTCTime, lastAttemptAt :: UTCTime, reason :: BadgeIssueFailure, current :: Bool}
```
...
**Store** (`Store/Badges.hs`): `UserBadgePurchase` gains `issueError :: Maybe BadgeIssueError` and `nextWakeAt :: Maybe UTCTime`. `getBadgePurchase` reads the three columns and the newest `badge_issuances.created_at` for the purchase; `issueError` is present when the three are, and `current` when `issue_error_at` is later than that time (or there is no issuance). `setBadgeIssueError db purchaseId now failure newRun`: `SET issue_error_at = ?, issue_error = ?`, and `issue_failed_since = ?` too when `newRun`. `setBadgeNextWake db purchaseId at_`.

**Worker** (`Library/Commands.hs`):

- `requestBadgeIssue` records every failing outcome through `setBadgeIssueError` before it returns or rethrows, with `newRun = maybe True (not . current) (issueError p)` from the purchase it was handed: the `BSPError` branch (`BIFServiceError code (isJust retryAfter)`), the unexpected-response branch (`BIFUnexpected`), a `Nothing` from `verifyIssuedCredential` (`BIFInvalidCredential`, alongside the statement it still applies), `applyBadgeStatement` answering `False` (`BIFUnexpected` with the internal error's text), and any throw from the request itself, caught with `catchAllErrors`, recorded by the classification above and rethrown so `retryBadgeError` still decides the retry. A successful issuance needs no write: its `badge_issuances` row is what makes the failure old.
