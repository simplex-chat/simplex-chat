...
**Files:** `src/Simplex/Chat/Library/Commands.hs`, `src/Simplex/Chat/Store/Badges.hs` (`checkStatementEntries`, §9), `apps/simplex-badge-service/src/BadgeService/Service.hs` and `tests/Bots/BadgeServiceTests.hs` (the funded-tier ruling, §9), `docs/protocol/badges-rpc.md`, `tests/Bots/BadgeManagerTests.hs` (two stub expectations)
...
- Send `purchaseBadge`, once. The request's `badgeInfo.badgeType` is not the client's to choose — a code carries no tier, so the service signs the tier the CODE funds and ignores the field on this command (the B7 change C4 makes, §9); the response states the tier the purchase row is written with.
...
- `createOrder`, writing the `@invoices` row and the `@web_orders` row that references it in one call, taking a `NewWebOrder` record carrying both rows' fields — including the `invoiceId`, which the caller mints, since the store mints no identifiers (§9).
...
- The two column enums the web tables need, `OrderMethod` and `WebOrderStatus`, are defined here with their `TextEncoding`/`ToField`/`FromField` instances — `OrderMethod` is the type D6's `Orders.hs` first named `Method`, moved because D0 has to persist a method before that module exists (§9). `@invoices.provider` is derived from the method and written through a real `TextEncoding PaymentProvider`, which this step adds, discharging B1's and C1's §9 notes.
...
**Files:** `apps/simplex-badge-service/web/{package.json,package-lock.json,tsconfig.json,build.mjs,index.html,styles.css,.gitignore}`, `apps/simplex-badge-service/web/src/`, `apps/simplex-badge-service/web/assets/.gitkeep`, `apps/simplex-badge-service/web/dist/`
...
**Files:** `apps/simplex-badge-service/web/{styles.css,index.html,package.json}`, `apps/simplex-badge-service/web/src/{router.ts,view.ts,ui.ts,main.ts}`, `apps/simplex-badge-service/web/test/{router.test.mjs,static.test.mjs,css.mjs}`, `apps/simplex-badge-service/web/assets/logo-symbol-{light,dark}.svg` (copied from `website/src/img/new/`)
...
- The shell is four modules, so that everything decidable without a browser can be asserted without one (§9): `router.ts` is the pure hash-to-screen mapping, `view.ts` describes each screen as a pure element tree, `ui.ts` is the only module that touches the DOM, and `main.ts` starts it. `npm test` runs the assertions on `node:test`, a built-in, so decision 7's one-devDependency cap is untouched. D3's money formatter belongs in the same pure half.
- A question screen does not advance when a radio's `change` fires: the arrow keys move the selection inside a radio group, so auto-advance puts every option after the first out of keyboard reach. Each question ends in a Continue button, and submitting one unanswered shows the error banner (§9).
...
**Verify:** In `apps/simplex-badge-service/web`: `npx tsc --noEmit` clean, `npm run build`, then `npm test` (`test/router.test.mjs` and `test/static.test.mjs`), then a second `npm run build` leaving `git status --porcelain -- apps/simplex-badge-service/web/dist` empty (D8). The assertions run against the **built** output and against the two files served from `web/`, never against `src/`: every custom property used anywhere has a definition on bare `:root` and the dark block introduces none of its own; the accent is exactly `#0053D0` and `#70F0F9`; `styles.css` carries no `@@…@@` token, no `url()` and no absolute URL; every screen renders exactly one `<h1>`; every radio group is a real `<fieldset>` with a real `<legend>` and every radio sits inside a `<label>`; a `:focus-visible` rule exists and no `outline: none` appears anywhere; the reduced-motion block turns off the 150 ms fade; the column is capped at `max-width: 560px`; all six hashes resolve and the built shell registers a `popstate` listener and never assigns markup from a string. Each of these was run against a deliberately broken input and observed to fail there (§9).

**Rendering is asserted nowhere, by anything, until the phase's browser pass.** That pass is one §9 entry covering D1's three deferred assertions and D2's own; it is not handed from step to step. Do not read a green `npm test` as evidence that the page draws.
...
- **C5 — the badge-service suite's echo assertion is drained, not compared, and that is a change to the WHOLE test tree.** `##>` asserted that the first line the virtual terminal produced after a command was typed equalled that command; on the badge-service suite it failed about one run in three, in untouched B10 examples, on the long `/_service_request` lines — a different example each time, each passing on rerun (C4's report, its fifth concern). `ChatTests.Utils` now DRAINS that echo instead: `drainEcho` consumes lines until they have spelled the command out, tolerating a leading `"> "` input-prompt prefix and fragments delivered in order, and reports the same mismatch as before for anything else (a line that is not a prefix of what was sent). The echo carries no information the test did not itself write, which is what makes draining it legitimate rather than a weakened assertion; no other assertion in the tree changed. **The mechanism was not reproduced**: seven consecutive runs in this step's session were clean — three of the badge-service suite before the change and four of the full covering suite after it — so the fix is a tolerance justified by C4's evidence rather than by a diagnosis of `readTerminalOutput`'s window diffing. If it recurs, the failure will now name the accumulated fragments.
- **C5 — `withBadgeServiceClock` is a projection of a new `withBadgeServiceAddress`.** Two of C5's cases cannot be written against a harness that owns the second service phase: one runs a client with the service NOT started (the transport-failure case), and one stops the service mid-example, clears a code's redemption in its database and starts it again with the SAME client still running (the `codes unredeem` case) — the service's SQLite file cannot be opened while the service holds it. `withBadgeServiceAddress` performs the first startup phase, reads BOTH published forms of the address, and hands the body the second phase as `withService :: IO () -> IO ()`. `withBadgeServiceClock` is now one call to it and every B10 call site is unchanged.
- **C5 — the full contact request URI is exercised, `CTName` and `CTDomain` are not.** `resolveServiceTarget` accepts four target forms; the short link is what every other example configures and the full URI now has its own redemption. The two name forms need a resolvable SimpleX name published in the test environment, which this harness cannot do — **recorded as untested rather than skipped silently.**
- **C5 — C3's `BadgeGate` is exported and reused, not copied.** The redeem path reads `badgeCurrentTime` at the same point a worker pass does — after the request has been answered and before the first row is written — so gating that clock parks a redemption in exactly the window `redeemBadgeCode`'s `activeUserId == Just userId` guard covers. `Bots.BadgeManagerTests` now exports `BadgeGate`, `newBadgeGate`, `gatedClockAt`, `allowPass` and `waitPasses`; `gatedClock` is `gatedClockAt passNow` and C3's own tests are unchanged. That example also runs the redemption OFF the terminal input loop (`execChatCommand'` on an `async`), because that loop is single-threaded and a `/create user` typed at it would queue behind the parked redemption instead of racing it.
- **C5 — six malformed-response cases needed a stub responder, and it was built.** A live service cannot answer with a null credential, a statement carrying no issued period, a credential with no expiry, a statement carrying an entry type this build cannot store, a response of the wrong shape for the command, or a body that is not a `BadgeServiceResponse` at all — and each of those is a branch of `redeemBadgeCode` whose whole purpose is to write nothing. `withBadgeStub` is a chat client with `clientService` set and `processServiceRequests` on, holding a DR contact address the client is pointed at; the test answers each `CEvtServiceRequest` with a canned body through `/_service_response`. Its credentials are really signed, with a `bbsKeyGen` key the client is configured with, so verification passes and the branch under test is the one that fails.
- **C5 — `codes unredeem` (H2) does not exist yet, so B1's `unredeemCode` is driven directly.** `BadgeService.Admin` has `issue`, `revoke` and `status` only. The recovery case — the same code redeeming again into a NEW purchase that supersedes the old one — calls the store function the H2 command will call, in the window where the service is stopped. When H2 lands it should re-point this example at the command.
- **C5 — codes are seeded with `seedTestCodes`, not through `runAdminCmd`.** B10's helper mints them with B3's generator and stores their hashes exactly as `codes issue` does; `codes issue` itself opens its own store and cannot run while the service holds the database, and it already has its own example (`testBadgeServiceCodesIssueRevokeStatus`).
- **C5 — the transport-failure case shortens the agent's `serviceRequestTimeout` to two seconds.** Its default is thirty, which is longer than any terminal assertion waits. What is pinned is what the client does AFTER the timeout, not what the default is.
- **C5 — `GADTs` is enabled in `Bots.BadgeServiceTests`.** `ConnectTarget` is a GADT and matching one without `MonoLocalBinds` raises `-Wgadt-mono-local-binds`; `Simplex.Chat.Library.Commands`, which matches the same type, enables it for the same reason. The module compiles clean with it.
- **C5 — `badgeCurrentTime` is now exercised on the CLIENT COMMAND path.** C2's entry above said C5 would be the first place the override is proved to work and C3's entry corrected that to C3, for the worker. C5 adds the other half: most examples drive the service clock and `badgeCurrentTime` from ONE `IORef`, so a redemption's stored badge status, its issuance period and the profile's paid-through date are all literal dates, and the worker example crosses a month boundary by moving that one ref.
- **C5 — five production mutations were run to prove the load-bearing assertions can fail.** Appending a suffix to `entry_uuid` in `insertLedgerEntries` fails the ledger parity comparison; deleting `verifyUserBadge` from `storeRedeemedBadge`, and deleting the pre-transaction `checkStatementEntries`, each turn their "writes nothing" example into a redemption that succeeds or raises the wrong error; rotating the master key in `issueDueBadgePeriod`'s request fails the master-key stability assertion with the two keys printed; and reverting the `activeUserId == Just userId` guard to an unconditional `chatWriteVar` fails the profile-switch example with `("active profile", 1)` where 2 was expected. A sixth, writing the `payments` row before the request is sent, fails the row counts of all three "writes nothing" examples with their error messages unchanged — which is what proves the counts themselves are load-bearing and not decoration on a message assertion. All reverted.
- **C5 — what remains untested.** The lock discipline (C3's and C4's own gap: the read-send-write scope and the release before `presentUserBadgeToContacts` still hold by source shape alone); `CTName`/`CTDomain` address forms, above; and the Postgres cross-check, which was not run for this step — it adds no SQL of its own, every write goes through C1's and B1's store functions, and the two `#if !defined(dbPostgres)` guards in the module are B10's, untouched.

- **C5 fix round — the unstorable-statement case covers all four entry types and asserts the whole message.** It stubbed `credit(charge)` only and stopped its assertion before `tshow e`, so a build that refused the WRONG entry type would still have passed. `unstorableEntryCases` now drives `credit(charge)`, `credit(transferIn)`, `debit(upgrade)` and `debit(transferOut)` — every type `storedEntryType` refuses — and each is compared as a complete line including the reason, spelled out literally rather than derived from the function under test. Verified live: swapping one expected reason for another's fails the example naming both.
...
- **D1 — the repository ignores `dist`, so the committed build needs `web/.gitignore` to re-include it.** `.gitignore:14` carries a bare `dist` under "# Haskell", which matches a directory of that name at any depth: `git add apps/simplex-badge-service/web/dist` was a silent no-op, and decision 7's committed artefact, D4's `embedDir` source and D8's gate would all have been comparing against a directory git never tracked. `web/.gitignore` now holds `node_modules/` and `!dist/`, following the per-package `.gitignore` convention of `packages/simplex-chat-webrtc` and `apps/simplex-support-bot` rather than editing the root file; a nested `.gitignore` overrides a parent's rule, and the negation works because no parent directory of `dist/` is itself excluded. Added to the step's **Files**. Anyone moving this directory must move the negation with it — `git status` will not report the loss.
- **D1 — the build is `build.mjs`, which the step's Files list omitted, and it checks two things `tsc` cannot.** The step describes the build (compile, copy `assets/`, emit `dev.html`) without naming a file for it; decision 7's one-devDependency cap makes it a node script over `fs`/`path` alone. Two build-time checks were added, both of which fail loudly rather than producing a page that is broken only in a browser, which is the one environment CI does not have:
  - **Relative import specifiers must resolve inside the build.** `tsc` does not rewrite specifiers (decision 7), so `import … from "./ui"` type-checks, emits, and then 404s in the browser leaving a blank page. Each emitted `.js` is scanned for relative specifiers and each must name an emitted file. This is the premise D4's single-prefix asset hash exists to protect, checked at the point it is created.
  - **Every `@@…@@` token must name a file in the served set or a known non-file token,** or the build fails naming the token and listing the set. This is D4's startup rule ("a token naming a file that is not in the served set fails at startup"), applied one stage earlier; D4 should implement the same rule rather than a second, looser one.
...
- **D2 — a radio's `change` may not navigate; each question needs a Continue button.** "One question per screen" reads like an invitation to advance as soon as an option is chosen, and that is inaccessible: arrow keys move the selection *within* a radio group, so a keyboard user pressing Down to hear the second option would be navigated away before reaching it, and every option after the first becomes unreachable — which is precisely what the step's own Verify line asks someone to confirm they can do. Each question therefore ends in a Continue button. Submitting one unanswered renders the error banner the step already requires for failed fetches, rather than a native `required` validation bubble, so there is one visible error surface on the site instead of two. D5's prefill skips a screen without any of this, since it never renders the question at all.
- **D2 — the two logos differ in intrinsic height (34 and 35), and light/dark names the scheme, not the ink.** `logo-symbol-light.svg` is the one shown in *light* mode (dark blue ink, `#023789`) and `logo-symbol-dark.svg` in dark mode (white ink); `website/src/_includes/navbar.html:200-201` shows the same pairing, and getting it backwards yields an invisible logo rather than an error. Their `viewBox`es are `0 0 34 34` and `0 0 34 35`, so the CSS sets `height` and leaves `width: auto` rather than fixing both, and each `<img>` carries its own intrinsic `width`/`height` attributes to reserve the right box before the file loads.
