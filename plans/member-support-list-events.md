...
- The member list loads only if `membersLoaded` is false. On Kotlin the load effect is keyed on `membersLoaded`, so a reset while the list is open (for example `ChatView` recomposing after an Android configuration change) reloads it. The mention picker already uses this flag the same way, and it is reset when leaving the group.
